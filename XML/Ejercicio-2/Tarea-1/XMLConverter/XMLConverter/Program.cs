        ...
        private static int AddElementWML(XmlReader xml, StringBuilder outputFile, int points)
        {
            ...
            switch (xml.Name)
            {
                case "gml:description":
                    ...
                    outputFile.AppendLine(value);
                    break;
                case "gml:beginPosition":                
                    value = xml.ReadElementContentAsString();
                    day = value.Substring(0, 10);
                    time = value.Substring(11, 8);
                    outputFile.AppendLine("Las mediciones han comenzado el día " + day + " a las " + time + "h");
                    break;
                ...
                case "wml2:time":
                    ...
                    day = value.Substring(0, 10);
                    time = value.Substring(11, 8);
                    outputFile.AppendLine("\nMedida número " + points + " realizada el día " + day + " a las " + time + "h");
                    break;
                case "wml2:value":
                    outputFile.AppendLine("\tCon valor: " + xml.ReadElementContentAsString());
                    break;
                case "gml:pos":
                    value = xml.ReadElementContentAsString();
                    String lat = value.Substring(0, 10);
                    String longitud = value.Substring(11, 8);
                    outputFile.AppendLine("\nCuya Latitud es " + lat + " y su Longitud es " + longitud);
                    break;
                case "wml2:zoneOffset":
                    value = xml.ReadElementContentAsString();
                    outputFile.AppendLine("Cuyo Huso Horario es " + value);
                    break;
                case "wml2:zoneAbbreviation":
                    value = xml.ReadElementContentAsString();
                    outputFile.AppendLine("Cuya Zona Horaria es " + value + "\n");
                    break;
                case "wml2:comment":
                    value = xml.ReadElementContentAsString();
                    outputFile.AppendLine("\t"+value);
